...
import { ProjectModal } from './HomePage';
...
const PortfolioPage: React.FC = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Simple filter matching category name or mapping
    return ALL_PROJECTS.filter(item => 
        item.category.toLowerCase().includes(filter.toLowerCase()) || 
        (filter === 'Web' && item.category.toLowerCase().includes('legal')) ||
        (filter === 'Web' && item.category.toLowerCase().includes('tech')) ||
        (filter === 'Web' && item.category.toLowerCase().includes('education'))
    );
  }, [filter]);
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                ...
                >
                    ...
                    <p className="max-w-2xl mx-auto text-slate-400 text-lg leading-relaxed mb-12">
                        Explore the precision and creativity of Sameer Digital Lab. From rapid 24-hour builds to large-scale platforms, we deliver excellence at every scale.
                    </p>
                </motion.div>
                    ...
                    <AnimatePresence mode='popLayout'>
                        {filteredItems.map((item, index) => (
                            ...
                            >
                                ...
                                <div className="relative h-full bg-slate-900 rounded-xl overflow-hidden border border-slate-800 group-hover:border-transparent transition-colors flex flex-col">
                                    {/* Image Area */}
                                    <div className="relative aspect-[4/3] w-full overflow-hidden cursor-pointer" onClick={() => setSelectedProject(item)}>
                                        <img 
                                            src={item.image1} 
                                            alt={item.title} 
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                <span className="text-slate-500 text-xs font-bold uppercase tracking-wide mb-1 block">{item.timeline}</span>
                                                <h3 className="text-xl font-bold text-white group-hover:text-cyan-400 transition-colors">{item.title}</h3>
                                            </div>
                                            <button 
                                                onClick={() => setSelectedProject(item)}
                                                className="bg-slate-800 p-2 rounded-full text-slate-400 group-hover:bg-cyan-500 group-hover:text-black transition-all"
                                            >
                                        ...
                                        <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-800">
                                            {item.tools.map((tool) => (
                                                <div key={tool} className="flex items-center gap-1.5 text-[10px] uppercase font-bold text-slate-500 bg-slate-950 border border-slate-800 px-2 py-1 rounded group-hover:text-cyan-400 group-hover:border-cyan-500/30 transition-colors">
                                                    {getToolIcon(tool, "w-3 h-3")}
                                                    {tool}
                                                </div>
                                            ))}
                                        </div>
